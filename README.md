...
The MAS runs on DALIA, which is a containerized launcher with a GUI for multi-agent-systems written in [DALI](https://github.com/AAAI-DISIM-UnivAQ/DALI). This means that the first step is to install this last, and to do this it is sufficient to follow the steps below, having listed pre-requisites.
...
2. Clone the [DALI](https://github.com/AAAI-DISIM-UnivAQ/DALI) repository and follow instructions to install DALI.
   Also, check the compatibility table [DALI-DALIA Compatibility](docs/compatibility.md) for DALI and DALIA versions.
...
4. Run the following command. By default, from the installation of SICStus, the first parameter should be `/usr/local/sicstus4.6.0`.
    ```sh
    ./run --sicstus <path_to_sicstus_directory> --dali ./dali --src ./src
    ```